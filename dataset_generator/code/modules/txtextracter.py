...
def record_data() -> None:
    ...
    with open("Hands/" + word + "/" + word + str(iteration) + ".txt", "w+") as f:
        write = csv.writer(f)
        write.writerows(final_hand_data)
    with open(
        "DataOriginal/Face/" + word + "/" + word + str(iteration) + ".txt", "w+"
    ) as f:
        write = csv.writer(f)
        write.writerows(final_face_data)
    with open("Torso/" + word + "/" + word + str(iteration) + ".txt", "w+") as f:
        write = csv.writer(f)
        write.writerows(final_pose_data)
    final_hand_data = []
    iteration += 1
    word = word_list[currentPosition]
    if iteration == len(get_ready_words("/videos/{word}/")) + 11:
        iteration = len(get_ready_words("/videos/{word}/"))
        currentPosition += 1
